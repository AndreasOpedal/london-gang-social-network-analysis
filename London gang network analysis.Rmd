...
author: "Andreas Opedal, Nicolò Grometto & Charlotte Out"
date: "23/05/2020"
output:
  html_document:
    df_print: paged
...
<i> DESCRIPTION: Data is on co-offending in a London-based inner-city street gang, 2005-2009, operating from a social housing estate. Data comes from anonymized police arrest and conviction data for all confirmed members of the gang.
...
We note the following of the network: \
1. The network data contains a large collection of attributes relative to the subjects who are part of the gang under consideration, e.g. age, birthplace, residence, number of arrests, number of convictions and more. In the following analysis we focus particularly on age and birthplace; \
2. The network data relative to the criminal gang under analysis is organised in layers, from members who only hang out together all the way up to members who have kin relationships and collaborate in serious crime acts, where upper layers are subsets of the lower. This particular feature allows to study the layered structure of this community and understand how interpersonal ties interact with crime acts in this kind of community; \


***

In what follows, we concentrate specifically on the hang-out network (ties coded as 1 and above) and on the Serious Crime Network (ties coded as 3 and above). Limiting our focus to a subset of the data, we prioritise the depth of our analysis, rather than breadth: \

We now proceed with the description, analysis and visual representation of the chosen data set.\

## Data Exploration

In this section, we proceed by calculating basic descriptives on the hang-out and co-offenders (serious crime) networks, with ties encoded by 1, 2, 3, and 4 as well as 3 and 4 in the network data, respectively. In particular, we are interested in analysing the data in the context provided by the associated attributes. We also provide appropriate visualisations of the different networks as well as comments and insights into our analysis, based on the obtained results.  
...
print(paste0("Proportion born in West Africa: ", round(west_africa, digits = 3)))
print(paste0("Proportion born in the Caribbeans: ", round(caribbean, digits = 3)))
print(paste0("Proportion born in the UK: ", round(uk, digits = 3)))
print(paste0("Proportion born in East Africa: ", round(east_africa, digits = 3)))
...
3. The gang in uneven in terms of ethnicity of the members, with more than 44% of its members being British. The remaining individuals are from  West Africa (22%), Caribbean (22%), and East Africa (11%). This prompts us to investigate further the degree of ethnic segregation both in the Hang-out and Serious Crime Networks, by conducting an appropriate cluster analysis; \
...
print(paste0("Same-ethnicity ties (proportion) - West Africa: ", round(west_africa_ratio_1, digits = 4)))
print(paste0("Same-ethnicity ties (proportion) - Caribbean: ", round(caribbean_ratio_1, digits = 4)))
print(paste0("Same-ethnicity ties (proportion) - UK: ", round(uk_ratio_1, digits = 4)))
print(paste0("Same-ethnicity ties (proportion) - East Africa: ", round(east_africa_ratio_1, digits = 4)))
print(paste0("Same-ethnicity ties - TOT (proportion): ", round(observed_et_1, digits = 4)))
...
print(paste0("Same-age ties (proportion) - 16 to 19: ", round(age_group_1_ratio_1, digits = 4)))
print(paste0("Same-age ties (proportion) - 20 to 24: ", round(age_group_2_ratio_1, digits = 4)))
print(paste0("Same-age ties (proportion) - above 24: ", round(age_group_3_ratio_1, digits = 4)))
print(paste0("Same-age ties - TOT (proportion): ", round(observed_age_1, digits = 4)))
...
3. Some evidence of birthplace segregation appears by inspection, especially with respect to ethnic group 1, namely West Africa, as previously considered (see plot 2 and corresponding considerations); \ 
...
We start by providing the same descriptives as for the Hang-Out Network in order to compare the two.
...
print(paste0("Same-ethnicity ties (proportion) - West Africa: ", round(west_africa_ratio_3, digits = 4)))
print(paste0("Same-ethnicity ties (proportion) - Caribbean: ", round(caribbean_ratio_3, digits = 4)))
print(paste0("Same-ethnicity ties (proportion) - UK: ", round(uk_ratio_3, digits = 4)))
print(paste0("Same-ethnicity ties (proportion) - East Africa: ", round(east_africa_ratio_3, digits = 4)))
print(paste0("Same-thnicity ties - TOT (proportion): ", round(observed_et_3, digits = 4)))
...
print(paste0("Same-age ties (proportion) - 20 to 24: ", round(age_group_2_ratio_1, digits = 4)))
print(paste0("Same-age ties (proportion) - above 24: ", round(age_group_3_ratio_1, digits = 4)))
print(paste0("Same-age ties - TOT (proportion): ", round(observed_age_3, digits = 4)))
...
We proceed to the second CUG test to compare same-ethnicity ties. For this test we choose the Serious Crime Network in light of what was noted above: there are considerably more same-ethnicity ties in the Serious Crime Network compared to the Hang-Out Network. \
...
In this section, we propose a function that uniformly generates random networks with a prescribed number of nodes, number of ties and number of isolates. After introducing the function, we will conduct and document a number of tests. The motivation is to then perform CUG tests like above which also conditions on the number of isolates in the network.
...
In this section we conduct two CUG tests, mimicking those performed on same-ethnicity ties and same-age group ties performed above, but keeping the number of isolated nodes constant. \
...
```{r}
# First re-write function from above, to simulate networks 
...
```{r}
# Recall the parameters of the Serious Crime Network: 54 nodes, 41 ties and 22 isolates
...
As seen in the output above the obtained p-value is `r round(p_value_age_const_iso, digits = 4)`, which is highly significant at the chosen type I error. Thus, we reject the null hypothesis. This provides evidence for the claim that gang members of the same age group are more likely to hang out together. The p-value is not identical to the one obtained above however, which is due to the randomness associated with simulating 1000 networks. If we instead compare the mean and standard deviations of the empirical distributions we see very similar values. The mean and standard deviation in the standard CUG test were `r sameagetie.mean` and `r sameagetie.sd` respectively. In the CUG test conditioned on number of isolates they are `r round(mean(same_age_prop), digits = 4)` and `r round(sd(same_age_prop), digits = 4)`. We can then (qualitatively) conclude that both samples come from the same distribution, as was argued above. In conclusion, the outcomes of our hypothesis tests in this section match those of the above one, with slight variations in the numercal expressions for p-values, as expected by randomness. This validates our previous conclusions, as well as the quality of our function for generating random networks with prescribed structural properties.
